...
Client::Client(void) {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec4) * mesh.vertexCount(), &(mesh.vertexArray()[0].x), GL_STATIC_DRAW);
  ...
  glBufferData(
      ...
      sizeof(cs200::Mesh::Edge) * mesh.edgeCount(),
      &(mesh.edgeArray()[0].index1),
      GL_STATIC_DRAW);
  ...
  const int COUNT = 8;
  float aspect = mesh.dimensions().x / mesh.dimensions().y, dx = (aspect < 1) ? 2.0f / COUNT : 2.0f * aspect / COUNT,
        dy = (aspect < 1) ? 2.0f / COUNT / aspect : 2.0f / COUNT,
        oscale = (aspect < 1) ? 0.95f * dx / mesh.dimensions().x : 0.95f * dy / mesh.dimensions().y;
  for (int i = 0; i < COUNT; ++i) {
    for (int j = 0; j < COUNT; ++j) {
      float sign_x = (i % 2 == 0) ? 1 : -1, sign_y = (j % 2 == 0) ? 1 : -1;
      glm::vec4 v = cs200::vector((i + 0.5f) * dx - 1, (j + 0.5f) * dy - 1);
      glm::mat4 obj2world =
          cs200::translate(v) * cs200::scale(sign_x * oscale, sign_y * oscale) * cs200::translate(O - mesh.center());
  ...
  fg_obj2world = ((aspect < 1) ? cs200::scale(1.0f / mesh.dimensions().y) : cs200::scale(1.0f / mesh.dimensions().x)) *
